...
async function yahooFinanceChart(symbol, range) {
...
async function marketChart(env, symbol, range) {
  ...
  try {
    const candles = await yahooFinanceChart(symbol, range);
...
async function alphaVantageEarnings(env, symbol) {
  ...
  return rows.slice(0, 12).map(row => {
    ...
    const priorRevenue = revenues.filter(item => {
      const elapsedDays = (time - item.time) / 86400000;
      return elapsedDays >= 300 && elapsedDays <= 430;
    }).reduce((best, item) => Math.abs((time - item.time) / 86400000 - 365.25) < Math.abs((time - (best?.time ?? -Infinity)) / 86400000 - 365.25) ? item : best, null);
    const revenue = revenueRow && Math.abs(revenueRow.time - time) <= 75 * 86400000 ? revenueRow.revenue : null;
    const revenueGrowthYoY = Number.isFinite(revenue) && Number.isFinite(priorRevenue?.revenue) && priorRevenue.revenue !== 0 ? (revenue / priorRevenue.revenue - 1) * 100 : null;
    return {
      symbol,
...
async function yahooEarnings(symbol, retry = true) {
  ...
  if (retry && (response.status === 401 || /crumb|unauthorized/i.test(errorMessage))) {
    await yahooSession(true);
    return yahooEarnings(symbol, false);
  }
  ...
  const revenues = statements.map(row => ({
    time: yahooRawNumber(row.endDate) * 1000,
    revenue: yahooRawNumber(row.totalRevenue)
  })).filter(row => Number.isFinite(row.time) && Number.isFinite(row.revenue) && row.revenue >= 0);
  ...
  return history.slice(0, 12).map((row, index) => {
    const timestamp = yahooRawNumber(row.quarter);
    const date = new Date(timestamp * 1000);
    const time = date.getTime();
    const revenueRow = revenues.reduce((best, item) => Math.abs(item.time - time) < Math.abs((best?.time ?? Infinity) - time) ? item : best, null);
    const priorRevenue = revenues.filter(item => {
    ...
    return {
      ...
      fiscalDateEnding: date.toISOString().slice(0, 10),
      reportedDate: null,
      actual: yahooRawNumber(row.epsActual),
      estimate: yahooRawNumber(row.epsEstimate),
      surprise: yahooRawNumber(row.epsDifference),
      revenue,
      revenueGrowthYoY: Number.isFinite(revenue) && Number.isFinite(priorRevenue?.revenue) && priorRevenue.revenue !== 0 ? (revenue / priorRevenue.revenue - 1) * 100 : index === 0 ? latestRevenueGrowth : null,
      revenueGrowthBasis: Number.isFinite(priorRevenue?.revenue) ? "Quarterly YoY" : index === 0 && Number.isFinite(latestRevenueGrowth) ? "Latest provider YoY" : null,
      provider: "Yahoo Finance"
    };
  });
}
...
async function stockPayload(env, url) {
  ...
  try {
    ...
    if (error.partialQuote && Number.isFinite(Number(error.partialQuote.c)) && Number(error.partialQuote.c) > 0) {
      ...
      quote = { unavailable: true, c: null, d: null, dp: null, h: null, l: null, o: null, pc: null, t: null, provider: "Unavailable", message: "No verified live quote was returned by the configured providers." };
      diagnostics.push({ provider: "Market data", ok: false, authApplied: false, message: "No generated quote or chart data was substituted for the unavailable provider response" });
    }
  }
  try {
  ...
  try {
    earnings = await alphaVantageEarnings(env, symbol);
    earningsProvider = "Alpha Vantage";
  } catch (error) {
    diagnostics.push({ provider: "Alpha Vantage earnings", ok: false, authApplied: Boolean(alphaVantageKey(env)), message: error.message });
    try {
      earnings = await yahooEarnings(symbol);
      earningsProvider = "Yahoo Finance";
      diagnostics.push({ provider: "Yahoo Finance earnings", ok: true, authApplied: false, message: "Reported EPS, estimates, and quarterly revenue returned" });
    } catch (fallbackError) {
      diagnostics.push({ provider: "Yahoo Finance earnings", ok: false, authApplied: false, message: fallbackError.message });
    }
  }
